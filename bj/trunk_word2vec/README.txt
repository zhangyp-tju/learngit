...
以上命令表示的是输入文件是test.txt，输出文件是vectors.bin，不使用cbow模型，默认为Skip-Gram模型。
...
#./word2vec3 -train zyp_traindata -output vec.txt -size 100 -window 5 -sample 1e-4 -negative 5 -hs 0 -binary 0 -cbow 1 -iter 3 -save-vocab zyp_td_df.txt -threads 5 -classes 200   
---- 聚类(-classes)的k=200,如何做到合适的程度呢？？ ---- 保存词频（-save-vocab）,就是统计词频，并建立词汇表 （即词频的统计，对于大文本我们可以交给集群来处理，之后直接用-read-vocab来读取词频词汇表）
#./word2vec3 -train zyp_traindata -output vec.txt -size 100 -window 5 -sample 1e-4 -negative 5 -hs 0 -binary 0 -cbow 1 -iter 3 -read-vocab zyp_td_df.txt -threads 5 -classes 1000 -debug 2    
---- debug 2 输出更多的训练信息
# 参数的默认值是 int binary = 0, cbow = 0, debug_mode = 2, window = 5,min_count = 5, num_threads = 1, min_reduce = 1;
#排序
sort -k2nr vec.txt > vec.txt.sort



./word2phrase -train word2vec_traindata  -output phrases.txt -threshold 100 -debug 2
请看word2phrase.c文件即可
